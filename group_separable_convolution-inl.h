...
*/
#ifndef MXNET_OPERATOR_GROUP_SEPARABLE_CONVOLUTION_INL_H_
#define MXNET_OPERATOR_GROUP_SEPARABLE_CONVOLUTION_INL_H_
...
namespace mxnet {
	namespace op {
		...
		namespace sepconv {
			enum GroupSeparableConvolutionOpInputs { kData, kWeight, kBias };
			enum GroupSeparableConvolutionOpOutputs { kOut };
			enum GroupSeparableConvolutionOpResource { kTempSpace };
			enum GroupSeparableConvolutionOpCudnnTune { kOff, kLimited, kFastest };
			...
			struct DepthwiseArgs {
				...
				DepthwiseArgs(int batch_t, int in_rows_t, int in_cols_t, int in_depth_t, int filter_rows_t, int filter_cols_t,
					int depth_multiplier_t, int in_depth_multiplier_t, int stride_y_t, int stride_x_t, int pad_rows_t, int pad_cols_t, int dilate_y_t,
					...
					in_depth_multiplier = in_depth_multiplier_t;
					stride_y = stride_y_t;
					stride_x = stride_x_t;
					pad_rows = pad_rows_t;
					pad_cols = pad_cols_t;
		...
		struct GroupSeparableConvolutionParam : public dmlc::Parameter<GroupSeparableConvolutionParam> {
			TShape kernel;
			TShape stride;
			TShape dilate;
			TShape pad;
			...
			DMLC_DECLARE_PARAMETER(GroupSeparableConvolutionParam) {
		...
		class GroupSeparableConvolutionOp : public Operator {
		public:
			explicit GroupSeparableConvolutionOp(GroupSeparableConvolutionParam p) {
				this->param_ = p;
			...
			virtual void Forward(const OpContext &ctx,
				...
				sepconv::DepthwiseArgs args(data.size(0), data.size(2), data.size(3), data.size(1), param_.kernel[0], param_.kernel[1],
					param_.num_filter / num_group, data.size(1) / num_group, param_.stride[0], param_.stride[1],
					param_.pad[0], param_.pad[1], param_.dilate[0], param_.dilate[1], out_rows,
					out_cols, param_.num_filter);

				GroupSeparableConv2dForward(out, data, weight, args);
			...
			virtual void Backward(const OpContext &ctx,
				...
				Tensor<xpu, 3, DType> gwmat = in_grad[sepconv::kWeight].get_with_shape<xpu, 3, DType>(wmat_shape, s);
				...
				GroupSeparableConv2dBackwardInput(gdata, grad, wmat, args);
				GroupSeparableConv2dBackwardFilter(grad, data, gwmat, args);
				...
				//if (param_.num_filter == num_group && use_mshadow_epx) {
				//	gwmat1 =
		...
		private:
			...
			index_t nstep_;
			GroupSeparableConvolutionParam param_;
		};  // class ConvolutionOp

		template<typename xpu>
		Operator* CreateOp(GroupSeparableConvolutionParam param, int dtype,
			std::vector<TShape> *in_shape,
			std::vector<TShape> *out_shape,
			Context ctx);

#if DMLC_USE_CXX11
		class GroupSeparableConvolutionProp : public OperatorProperty {
		public:
			...
			bool InferShape(std::vector<TShape> *in_shape,
				...
				if (param_.kernel.ndim() == 2) {
					...
					int num_group = param_.num_group;// dshape[1];
			...
			OperatorProperty* Copy() const override {
				auto ptr = new GroupSeparableConvolutionProp();
				ptr->param_ = param_;
				return ptr;
			}

			std::string TypeString() const override {
				return "GroupSeparableConvolution";
			}
