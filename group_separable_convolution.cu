...
namespace mshadow {
	namespace cuda {
		...
		// in NCHW format.
		template <typename T>
		__global__ void GroupSeparableConv2dGPUKernelNCHW(const mxnet::op::sepconv::DepthwiseArgs args,
			...
			const int in_depth_multiplier = args.in_depth_multiplier;
			...
			for (int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
				...
				int input_offset_temp = (OB * in_depth + in_d*in_depth_multiplier) * (in_rows * in_cols);
				...
				T sum = 0;
				int filter_offset_temp = (multiplier + depth_multiplier * in_d)*in_depth_multiplier* filter_rows * filter_cols;

				for (int i = 0; i < in_depth_multiplier; ++i) {
					...
					input_offset_temp += in_rows*in_cols;
					filter_offset_temp += filter_rows * filter_cols;
				}
				output[index] = sum;
			}
		}
		...
		template <typename T>
		__global__ void
			GroupSeparableConv2dBackpropInputGPUKernelNCHW(const mxnet::op::sepconv::DepthwiseArgs args,
			...
			for (int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
				...
				const int in_base = in_d / in_depth_multiplier;
				const int in_multiplier = in_d % in_depth_multiplier;

				T sum = 0;
				if (dilate_x == 1 && dilate_y == 1) {
					const int out_d_start = in_base * depth_multiplier;
					...
					for (int out_d = out_d_start; out_d < out_d_end; ++out_d) {
						const int filter_offset_temp = out_d * in_depth_multiplier * filter_rows * filter_cols + in_multiplier*filter_rows*filter_cols;
		...
		template <typename T>
		__global__ void GroupSeparableConv2dBackpropFilterGPUKernelNCHW2(
			...
			for (int index = (blockIdx.x + blockIdx.y * gridDim.x) * blockDim.x + threadIdx.x;
				...
				const int f_m = (index / filter_cols / filter_rows) % in_depth_multiplier;
				const int out_d = (index / filter_cols / filter_rows / in_depth_multiplier) % out_depth;

				const int in_base = out_d / depth_multiplier;
				//const int dm = out_d % depth_multiplier;
				const int in_d = in_base * in_depth_multiplier + f_m;
		...
		// a bit slow
		template <typename T>
		__global__ void GroupSeparableConv2dBackpropFilterGPUKernelNCHW(
			const mxnet::op::sepconv::DepthwiseArgs args, const T* out_backprop, const T* input,
			T* filter_backprop, int count) {
			const int in_rows = args.in_rows;
			const int in_cols = args.in_cols;
		...
		inline void GroupSeparableConv2dForward(const Tensor<gpu, 4, Dtype> &out,
			...
			CheckLaunchParam(dimGrid, dimBlock, "GroupSeparableConvolution Forward");
			cudaStream_t stream = Stream<gpu>::GetStream(out.stream_);
			GroupSeparableConv2dGPUKernelNCHW<Dtype> << <dimGrid, dimBlock, 0, stream >> >(args, bottom_data, weight_data, top_data, count);
			SEPARABLE_CONV_CUDA_CHECK(cudaPeekAtLastError());
		}

		template<typename Dtype>
		inline void GroupSeparableConv2dBackwardInput(const Tensor<gpu, 4, Dtype> &in_grad,
			...
			dim3 dimGrid(kMaxGridNum, (grid_size + kMaxGridNum - 1) / kMaxGridNum);
			dim3 dimBlock(kMaxThreadsPerBlock);
			CheckLaunchParam(dimGrid, dimBlock, "GroupSeparableConvolution Backward Input");
			cudaStream_t stream = Stream<gpu>::GetStream(in_grad.stream_);
			GroupSeparableConv2dBackpropInputGPUKernelNCHW<Dtype> << <dimGrid, dimBlock, 0, stream >> >(args, top_diff, weight_data, bottom_diff, count);
			SEPARABLE_CONV_CUDA_CHECK(cudaPeekAtLastError());
		}

		template<typename Dtype>
		inline void GroupSeparableConv2dBackwardFilter(const Tensor<gpu, 4, Dtype> &out_grad,
			...
			dim3 dimGrid(kMaxGridNum, (grid_size + kMaxGridNum - 1) / kMaxGridNum);
			dim3 dimBlock(kMaxThreadsPerBlock);
			CheckLaunchParam(dimGrid, dimBlock, "GroupSeparableConvolution Backward Filter");
			...
			GroupSeparableConv2dBackpropFilterGPUKernelNCHW2<Dtype> << <dimGrid, dimBlock, 0, stream >> >(args, top_diff, bottom_data, weight_diff, count);
			SEPARABLE_CONV_CUDA_CHECK(cudaPeekAtLastError());
		}
	...
	inline void GroupSeparableConv2dForward(const Tensor<gpu, 4, Dtype> &out,
		...
		const Tensor<gpu, 3, Dtype> &wmat,
		const mxnet::op::sepconv::DepthwiseArgs args) {
		cuda::GroupSeparableConv2dForward(out, data, wmat, args);
	}
	...
	inline void GroupSeparableConv2dBackwardInput(const Tensor<gpu, 4, Dtype> &in_grad,
		...
		const Tensor<gpu, 3, Dtype> &wmat,
		const mxnet::op::sepconv::DepthwiseArgs args) {
		cuda::GroupSeparableConv2dBackwardInput(in_grad, out_grad, wmat, args);
	}
	...
	inline void GroupSeparableConv2dBackwardFilter(const Tensor<gpu, 4, Dtype> &out_grad,
		...
		cuda::GroupSeparableConv2dBackwardFilter(out_grad, input, wmat_diff, args);
	}
...
namespace mxnet {
	namespace op {
		...
		template<>
		Operator* CreateOp<gpu>(GroupSeparableConvolutionParam param, int dtype,
			...
			MSHADOW_REAL_TYPE_SWITCH(dtype, DType, {
				op = new GroupSeparableConvolutionOp<gpu, DType>(param);
			})
